//
//  VOGOPlayer.h
//  VOGOPlayer
//
//  Created by Eric on 04/01/2018.
//  Copyright © 2018 VOGO. All rights reserved.
//
#import <UIKit/UIKit.h>

//! Project version number for VOGOPlayerCore.
FOUNDATION_EXPORT double VOGOPlayerCoreVersionNumber;

//! Project version string for VOGOPlayerCore.
FOUNDATION_EXPORT const unsigned char VOGOPlayerCoreVersionString[];

//
#import <VOGOPlayerCore/VGPlayer.h>
#import <VOGOPlayerCore/VGChannel.h>
#import <VOGOPlayerCore/VGDefines.h>
#import <VOGOPlayerCore/VGEaglUIView.h>
